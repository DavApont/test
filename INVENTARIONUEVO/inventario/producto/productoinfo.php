...
class cproducto extends cTable {
	...
	function __construct() {
		...
		// codigo_producto
		$this->codigo_producto = new cField('producto', 'producto', 'x_codigo_producto', 'codigo_producto', '`codigo_producto`', '`codigo_producto`', 3, -1, FALSE, '`codigo_producto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->codigo_producto->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['codigo_producto'] = &$this->codigo_producto;

		// codigo_departamento
		$this->codigo_departamento = new cField('producto', 'producto', 'x_codigo_departamento', 'codigo_departamento', '`codigo_departamento`', '`codigo_departamento`', 3, -1, FALSE, '`codigo_departamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->codigo_departamento->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['codigo_departamento'] = &$this->codigo_departamento;

		// nombre_producto
		$this->nombre_producto = new cField('producto', 'producto', 'x_nombre_producto', 'nombre_producto', '`nombre_producto`', '`nombre_producto`', 200, -1, FALSE, '`nombre_producto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nombre_producto'] = &$this->nombre_producto;

		// stop_min
		$this->stop_min = new cField('producto', 'producto', 'x_stop_min', 'stop_min', '`stop_min`', '`stop_min`', 200, -1, FALSE, '`stop_min`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['stop_min'] = &$this->stop_min;

		// stop_max
		$this->stop_max = new cField('producto', 'producto', 'x_stop_max', 'stop_max', '`stop_max`', '`stop_max`', 200, -1, FALSE, '`stop_max`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['stop_max'] = &$this->stop_max;

		// cantidad_existencia
		$this->cantidad_existencia = new cField('producto', 'producto', 'x_cantidad_existencia', 'cantidad_existencia', '`cantidad_existencia`', '`cantidad_existencia`', 200, -1, FALSE, '`cantidad_existencia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['cantidad_existencia'] = &$this->cantidad_existencia;

		// status_producto
		$this->status_producto = new cField('producto', 'producto', 'x_status_producto', 'status_producto', '`status_producto`', '`status_producto`', 200, -1, FALSE, '`status_producto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function SqlFrom() { // From
		return "`producto` LEFT JOIN `departamento` ON `departamento`.`codigo_departamento`=`producto`.`codigo_departamento`";
	}
	...
	// DELETE statement
		...
		$sql = "UPDATE " . $this->UpdateTable . " SET  status_producto='Inactivo'";
	...
	function SqlKeyFilter() {
		return "`codigo_producto` = @codigo_producto@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->codigo_producto->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@codigo_producto@", ew_AdjustSql($this->codigo_producto->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->codigo_producto->CurrentValue)) {
			$sUrl .= "codigo_producto=" . urlencode($this->codigo_producto->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["codigo_producto"]; // codigo_producto
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->codigo_producto->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->codigo_producto->Exportable) $Doc->ExportCaption($this->codigo_producto);
				...
				if ($this->stop_min->Exportable) $Doc->ExportCaption($this->stop_min);
				if ($this->stop_max->Exportable) $Doc->ExportCaption($this->stop_max);
				if ($this->cantidad_existencia->Exportable) $Doc->ExportCaption($this->cantidad_existencia);
				if ($this->status_producto->Exportable) $Doc->ExportCaption($this->status_producto);
			} else {
				if ($this->codigo_producto->Exportable) $Doc->ExportCaption($this->codigo_producto);
				if ($this->codigo_departamento->Exportable) $Doc->ExportCaption($this->codigo_departamento);
				if ($this->nombre_producto->Exportable) $Doc->ExportCaption($this->nombre_producto);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->codigo_producto->Exportable) $Doc->ExportField($this->codigo_producto);
					...
					if ($this->stop_min->Exportable) $Doc->ExportField($this->stop_min);
					if ($this->stop_max->Exportable) $Doc->ExportField($this->stop_max);
					if ($this->cantidad_existencia->Exportable) $Doc->ExportField($this->cantidad_existencia);
					if ($this->status_producto->Exportable) $Doc->ExportField($this->status_producto);
				} else {
					if ($this->codigo_producto->Exportable) $Doc->ExportField($this->codigo_producto);
					if ($this->codigo_departamento->Exportable) $Doc->ExportField($this->codigo_departamento);
					if ($this->nombre_producto->Exportable) $Doc->ExportField($this->nombre_producto);
