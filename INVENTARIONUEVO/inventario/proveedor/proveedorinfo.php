...
class cproveedor extends cTable {
	...
	function __construct() {
		...
		// rif
		$this->rif = new cField('proveedor', 'proveedor', 'x_rif', 'rif', '`rif`', '`rif`', 200, -1, FALSE, '`rif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// telefono
		$this->telefono = new cField('proveedor', 'proveedor', 'x_telefono', 'telefono', '`telefono`', '`telefono`', 200, -1, FALSE, '`telefono`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['telefono'] = &$this->telefono;

		// razon_social
		$this->razon_social = new cField('proveedor', 'proveedor', 'x_razon_social', 'razon_social', '`razon_social`', '`razon_social`', 200, -1, FALSE, '`razon_social`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['razon_social'] = &$this->razon_social;

		// persona_contacto
		$this->persona_contacto = new cField('proveedor', 'proveedor', 'x_persona_contacto', 'persona_contacto', '`persona_contacto`', '`persona_contacto`', 200, -1, FALSE, '`persona_contacto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['persona_contacto'] = &$this->persona_contacto;

		// status_proveedor
		$this->status_proveedor = new cField('proveedor', 'proveedor', 'x_status_proveedor', 'status_proveedor', '`status_proveedor`', '`status_proveedor`', 200, -1, FALSE, '`status_proveedor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['status_proveedor'] = &$this->status_proveedor;
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		$sql = "UPDATE " . $this->UpdateTable . " SET  status_proveedor='Inactivo'";
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->persona_contacto->Exportable) $Doc->ExportCaption($this->persona_contacto);
				if ($this->status_proveedor->Exportable) $Doc->ExportCaption($this->status_proveedor);
			} else {
				if ($this->rif->Exportable) $Doc->ExportCaption($this->rif);
				if ($this->direccion->Exportable) $Doc->ExportCaption($this->direccion);
				if ($this->telefono->Exportable) $Doc->ExportCaption($this->telefono);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->persona_contacto->Exportable) $Doc->ExportField($this->persona_contacto);
					if ($this->status_proveedor->Exportable) $Doc->ExportField($this->status_proveedor);
				} else {
					if ($this->rif->Exportable) $Doc->ExportField($this->rif);
					if ($this->direccion->Exportable) $Doc->ExportField($this->direccion);
					if ($this->telefono->Exportable) $Doc->ExportField($this->telefono);
