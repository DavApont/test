<?php
			if($_POST['buscar_user']){
				$result=mysql_query("SELECT * FROM user WHERE ID_user='".$_POST['login']."';",$conexion);
...
<table width="223" border="0" cellpadding="0" cellspacing="0" class="tablerouded">
  ...
  <tr>
    ...
    <td width="156"> <select name="login" class="normalText" id="login">
<?php  
$result=mysql_query("SELECT ID_user,usuario FROM user order by usuario",$conexion);
while($row = mysql_fetch_array($result)) {
?>
