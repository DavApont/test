...
class cusuarios_add extends cusuarios {
	...
	function ValidateForm() {
		...
		if (!$this->contrasena->FldIsDetailKey && !is_null($this->contrasena->FormValue) && $this->contrasena->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->contrasena->FldCaption());
		}
...
fusuariosadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($usuarios->contrasena->FldCaption()) ?>");
...
?>
<form name="fusuariosadd" id="fusuariosadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="text" data-field="x_usuario" name="x_usuario" id="x_usuario" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($usuarios->usuario->PlaceHolder) ?>" value="<?php echo $usuarios->usuario->EditValue ?>"<?php echo $usuarios->usuario->EditAttributes() ?>>
</span>
...
<?php if ($usuarios->contrasena->Visible) { // contrasena ?>
	<tr id="r_contrasena">
		<td><span id="elh_usuarios_contrasena"><?php echo $usuarios->contrasena->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $usuarios->contrasena->CellAttributes() ?>>
<span id="el_usuarios_contrasena" class="control-group">
<input type="text" data-field="x_contrasena" name="x_contrasena" id="x_contrasena" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($usuarios->contrasena->PlaceHolder) ?>" value="<?php echo $usuarios->contrasena->EditValue ?>"<?php echo $usuarios->contrasena->EditAttributes() ?>>
</span>
<?php echo $usuarios->contrasena->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
